...
delta_0 = np.zeros(no_gen)
delta_w0 = np.zeros(no_gen)
t_arr = np.zeros((no_of_iter))
Em = np.zeros(no_gen)

delta = np.zeros(no_gen)
delta_w = np.zeros(no_gen)

for i in range(no_gen):
    Delta[0][i] = cmath.phase(E[i])
    delta_0[i] = Delta[0][i]
    delta[i] = delta_0[i]
    Em[i] = abs(E[i])
...
delta_p = np.zeros(no_gen)
delta_wp = np.zeros(no_gen)

d_delta_dt_2 = np.zeros(no_gen)
d_deltaw_dt_2 = np.zeros(no_gen)

Pe = np.zeros(no_gen)
Pe_p = np.zeros(no_gen)

E_p = np.zeros(no_gen, dtype=complex)
Ig_p = np.zeros(no_gen)

fault_bus = 2
fault_Y = 1 / complex(0, 0.001)

Yred_prefault = Yred

Ynn[fault_bus][fault_bus] += fault_Y

Yred_fault = form_Yred()

for i in range(1, no_of_iter):
    if i < fault_init_iterno:
        Yred = Yred_prefault
    elif i < fault_remvl_iterno and i >= fault_init_iterno:
        Yred = Yred_fault
    else:
        Yred = Yred_prefault
    ...
    for j in range(no_gen):
        ...
        d_delta_dt_1[j] = delta_w[j]
        d_deltaw_dt_1[j] = math.pi * fo * (Pm[j] - Pe[j] - D[j] * delta_w[j]) / H[j]

        delta_p[j] = delta[j] + d_deltaw_dt_1[j] * del_t
        delta_wp[j] = delta_w[j] + d_deltaw_dt_1[j] * del_t

        E_p[j] = complex(Em[j] * math.cos(delta_p[j]), Em[j] * math.sin(delta_p[j]))

    Ig_p = find_Ig_frm_Yred(Yred, E_p)

    for j in range(no_gen):
        Sj = E_p[j] * np.conj(Ig_p[j])
        Pe_p[j] = Sj.real

        d_delta_dt_2[j] = delta_wp[j]
        d_deltaw_dt_2[j] = math.pi * fo * (Pm[j] - Pe_p[j] - D[j] * delta_wp[j]) / H[j]

        delta[j] = delta[j] + 0.5 * (d_delta_dt_1[j] + d_delta_dt_2[j]) * del_t
        delta_w[j] = delta_w[j] + 0.5 * (d_deltaw_dt_1[j] + d_deltaw_dt_2[j]) * del_t

        E[j] = complex(Em[j] * math.cos(delta[j]), Em[j] * math.sin(delta[j]))
...
f = np.empty((no_gen, no_of_iter))

for i in range(no_gen):
    for j in range(no_of_iter):
        f[i][j] = fo + Delta_w[i][j] / (2 * math.pi)
