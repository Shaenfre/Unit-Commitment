...
def find_overloaded_fdr():
    fd_S_overload = np.zeros(no_fdr - 1)
    k = 1
    is_any_ovrld = False
    for i in range(no_fdr):
        if fd_S_max_case1[i] < fd_S_max_case2[i]:
            fd_S_overload = i
            ++k
            is_any_ovrld = True
    return fd_S_overload, is_any_ovrld
...
line_no = 3
n = from_bus_col_feeder[line_no]
m = to_bus_col_feeder[line_no]
...
fdr_I = calc_i_frm_fdr()
fd_S, fd_S_max_case2 = calc_line_pwr()
print(fd_S_max_case2)
...
fd_S_overload, is_any = find_overloaded_fdr()
# print(fd_S_overload)
if is_any:
    print(f"overloaded lines after removal of line {line_no}")
    for i in range(no_fdr - 1):
        if fd_S_overload > 0:
            given_line = fd_S_overload[i]
            print(f"The line {given_line} is overloaded")
else:
    print(f"no fdr gets overloaded after removal of line {line_no}")
...
def form_load_admtnc():
    ...
    for i in range(no_bus):
        if Pd[i] != 0 or Qd[i] != 0:
           Sdi = complex(Pd[i], Qd[i])
           Idi = np.conj(Sdi / cmplx_vltg_vec[i])
...
def modify_Ybus_with_gen_admtnc():
    for i in range(no_gen):
        n = gen_bus[i]
        Y_bus_matrix[n][n] += Yg[i]
    return Y_bus_matrix

def modify_Ybus_with_load_admtnc():
    for i in range(no_bus):
        if yd[i] != 0:
            Y_bus_matrix[i][i] += yd[i]
    return Y_bus_matrix

def remove_load_admtnc_frm_Ybus():
    for i in range(no_bus):
        if yd[i] != 0:
            Y_bus_matrix[i][i] -= yd[i]
    return Y_bus_matrix

Y_bus_matrix = modify_Ybus_with_load_admtnc()
Y_bus_matrix = modify_Ybus_with_gen_admtnc()
# print(Y_bus_matrix)

I = np.zeros(no_bus)
I = np.dot(Y_bus_matrix, cmplx_vltg_vec)
# print(I)
# print(cmplx_vltg_vec)
...
Zf = 0.01
If = cmplx_vltg_vec[k] / (Z[k][k] + Zf)
# print(Z[1][1])
V_fault = cmplx_vltg_vec
...
def calc_Ig_Sg():
    Y_bus_matrix = remove_load_admtnc_frm_Ybus()
    Ig = np.dot(Y_bus_matrix, V_fault)
    Sg = np.zeros(no_bus, dtype=complex)
    for i in range(no_gen):
        gen_bus_no = gen_bus[i]
        Sg[gen_bus_no] = V_fault[gen_bus_no] * np.conj(Ig[gen_bus_no])
    return Ig, Sg
...
def check_KCL():
    sum_I = np.zeros((no_bus, 2), dtype=complex)
    for i in range(no_fdr):
        m = from_bus_col_feeder[i]
        n = to_bus_col_feeder[i]
    ...
    for i in range(no_tr):
        m = from_bus_col_transf[i]
        n = to_bus_col_transf[i]
