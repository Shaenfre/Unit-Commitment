...
I = np.zeros(no_bus)
I1 = np.dot(Y_bus_matrix, cmplx_vltg_vec)

# I2 = [[sum(a * b for a, b in zip(A_row, B_col))
#                         for B_col in zip(*cmplx_vltg_vec)]
#                                 for A_row in Y_bus_matrix]
...
def check_KCL():
    sum_I = np.zeros(no_bus, dtype=complex)
    for i in range(no_fdr):
...
def find_E():
    ...
    for i in range(no_gen):
        E[i] = cmplx_vltg_vec[i] + Ii[i] * Yg[i]
...
def find_Ig_frm_Yred():
    Ig_frmYred = np.dot(Yred, E)

    return Ig_frmYred

Ig_frmYred = find_Ig_frm_Yred()
# print(Ig)
# print(Ig_frmYred)
